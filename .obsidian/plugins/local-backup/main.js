            ...
            self.fs.open(path2, "w", 438, function(err2, fd) {
              if (err2) {
                self.fs.chmod(path2, 438, function() {
                  self.fs.open(path2, "w", 438, function(retryErr, fd2) {
      ...
      function findSync(dir, pattern, recursive, visited) {
        ...
        self.fs.readdirSync(dir).forEach(function(file) {
          ...
          if (stat.isDirectory() && recursive) {
            const realDir = self.fs.realpathSync(path3);
            if (!visited.has(realDir)) {
              visited.add(realDir);
              files = files.concat(findSync(path3, pattern, recursive, visited));
            }
          }
        });
        return files;
      }
      return findSync(path2, void 0, true, /* @__PURE__ */ new Set([self.fs.realpathSync(path2)]));
    };
...
var require_inflater = __commonJS({
  "node_modules/adm-zip/methods/inflater.js"(exports, module2) {
    var _a, _b;
    var version = +((_b = (_a = process == null ? void 0 : process.versions) == null ? void 0 : _a.node) != null ? _b : "").split(".")[0] || 0;
...
var LocalBackupSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Windows output path (optional)").setDesc(`${backupPathEnvDescBase} Example: D:\\Backups\\Obsidian`).addText(
...
var LocalBackupUtils = class {
   ...
   */
  createFileByArchiver(archiverType, archiverPath, archiveFileType, vaultPath, backupFilePath, customizedArguments) {
    const excludedPatterns = this.parsePatternList(
      this.plugin.settings.excludedDirectoriesValue
    );
    const includedPatterns = this.parsePatternList(
    ...
    const args = [];
    const cwdPath = includedPatterns.length > 0 ? vaultPath : void 0;
    const customArgsArr = customizedArguments ? customizedArguments.split(" ").filter((p) => p.trim() !== "") : [];
    if (excludedPatterns.length > 0) {
      this.log(
...
var LocalBackupPlugin = class extends import_obsidian3.Plugin {
  ...
  async loadSettings() {
    ...
    if (r && typeof r.backupOutputPathValue === "string") {
      ...
      if (unified) {
        if (!((_a = merged.winSavePathValue) == null ? void 0 : _a.trim())) {
          merged.winSavePathValue = unified;
        }
        if (!((_b = merged.unixSavePathValue) == null ? void 0 : _b.trim())) {
  ...
  async archiveVaultAsync(specificFileName) {
    try {
      ...
      const platformPath = platform === "win32" ? this.settings.winSavePathValue : this.settings.unixSavePathValue;
