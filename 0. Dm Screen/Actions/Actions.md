...
Special Icons denote these actions.
```ad-warning
title: Action Icons
collapse: open
icon: lucide-book-open
`pf2:1` Single Action
`pf2:2` Two-Action Activity
`pf2:3` Three-Action Activity
`pf2:r` Reaction
`pf2:0` Free Action
```
...
## `pf2:1` Single Actions
Single actions use this symbol: `pf2:1`. They're the simplest, most common type of action.  You can use three single actions on your turn in an encounter, in any order you see fit.

## `pf2:r` Reactions
Reactions use this symbol: `pf2:r`. These actions can be used even when it's not your turn. You only get one reaction per encounter round, and you can use it only when its specific trigger is fulfilled. Often, the trigger is another creature's action.

## `pf2:0` Free Actions
Free actions use this symbol: `pf2:0`. Free actions don't require you to spend any of your three single actions or your reaction. A free action might have a trigger like a reaction does. If so, you can use it just like a reaction - even if it's not your turn. However, you can use only one free action per trigger, so if you have multiple free actions with the same trigger, you have to decide which to use. If a free action doesn't have a trigger, you use it like a single  action, just without spending any of your actions for the turn.
...
Activities that use two actions use this symbol: `pf2:2`. Activities that use three actions use this symbol: `pf2:3`. A few special activities, such as spells you can cast in an instant, use a free action or a reaction.
